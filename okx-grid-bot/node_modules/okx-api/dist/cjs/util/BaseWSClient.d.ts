...
import { WsTopicRequest, WsTopicRequestOrStringTopic } from './websocket-util.js';
import WsStore from './WsStore.js';
import { WSConnectedResult } from './WsStore.types.js';
export interface WSClientEventMap<WsKey extends string, TWSUpdateEventData = any> {
    ...
    exception: (response: TWSUpdateEventData & {
        wsKey: WsKey;
    }) => void;
...
export interface BaseWebsocketClient<TWSKey extends string, TWSRequestEvent extends object, TWSUpdateEventData = any> {
    on<U extends keyof WSClientEventMap<TWSKey, TWSUpdateEventData>>(event: U, listener: WSClientEventMap<TWSKey, TWSUpdateEventData>[U]): this;
    emit<U extends keyof WSClientEventMap<TWSKey, TWSUpdateEventData>>(event: U, ...args: Parameters<WSClientEventMap<TWSKey, TWSUpdateEventData>[U]>): boolean;
}
...
export declare abstract class BaseWebsocketClient<TWSKey extends string, TWSRequestEvent extends object> extends EventEmitter {
    ...
    protected abstract getWsAuthRequestEvent(wsKey: TWSKey, skipPublicWsKeyCheck: boolean): Promise<object | null>;
     ...
     */
    protected abstract getWsRequestEvents(operation: WSOperation, requests: WsTopicRequest<string>[], wsKey: TWSKey): Promise<MidflightWsRequestEvent<TWSRequestEvent>[]>;
    /**
     ...
     */
    executeReconnectableClose(wsKey: TWSKey, reason: string): void;
     ...
     */
    assertIsAuthenticated(wsKey: TWSKey, skipPublicWsKeyCheck: boolean): Promise<unknown>;
}
