...
class BaseWebsocketClient extends events_1.default {
    ...
    async subscribeTopicsForWsKey(wsTopicRequests, wsKey) {
        ...
        const isAuthenticatedBeforeTopicCheck = this.wsStore.get(wsKey)?.isAuthenticated;
        if (!isAuthenticatedBeforeTopicCheck && !isPrivateConnection) {
            ...
            for (const topicRequest of normalisedTopicRequests) {
                if (websocket_util_js_1.PRIVATE_CHANNELS.includes(topicRequest.topic)) {
                    await this.assertIsAuthenticated(wsKey, false);
                    break;
                }
                if (websocket_util_js_1.PUBLIC_CHANNELS_WITH_AUTH.includes(topicRequest.topic)) {
    ...
    executeReconnectableClose(wsKey, reason) {
        ...
        if (wasOpen) {
            (0, websocket_util_js_1.safeTerminateWs)(this.wsStore.getWs(wsKey), true);
        }
        if (!wasOpen) {
    ...
    async sendAuthRequest(wsKey, skipPublicWsKeyCheck) {
        try {
            ...
            const request = await this.getWsAuthRequestEvent(wsKey, skipPublicWsKeyCheck);
    ...
    async onWsOpen(event, wsKey, url, ws) {
        ...
        for (const topicRequest of topicsForWsKey) {
            if (websocket_util_js_1.PUBLIC_CHANNELS_WITH_AUTH.includes(topicRequest.topic) ||
                websocket_util_js_1.PRIVATE_CHANNELS.includes(topicRequest.topic)) {
        ...
        const { privateReqs, publicReqs } = this.sortTopicRequestsIntoPublicPrivate(topicsForWsKey, wsKey);
     ...
     */
    async assertIsAuthenticated(wsKey, skipPublicWsKeyCheck) {
        const isConnected = this.getWsStore().isConnectionState(wsKey, WsStore_types_js_1.WsConnectionStateEnum.CONNECTED);
        if (!isConnected) {
