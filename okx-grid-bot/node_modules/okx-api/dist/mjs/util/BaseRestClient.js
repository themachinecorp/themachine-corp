...
import { getRestBaseUrl, programId, programKey, serializeParams, } from './requestUtils.js';
...
//   return response;
// });
export const MISSING_CREDENTIALS_ERROR = 'Private endpoints require api and secret to be provided in the REST client constructor';
export default class BaseRestClient {
    ...
    constructor(options = {}, requestOptions = {}) {
        ...
        if (options.market === 'demo') {
            throw new Error('ERROR: to use demo trading, set the "demoTrading: true" flag in the constructor');
        }
    ...
    postPrivate(endpoint, params) {
        return this._call('POST', endpoint, Array.isArray(params)
            ...
            : { ...params, [programKey]: programId }, false);
    }
    ...
    async _call(method, endpoint, params, isPublicApi) {
        ...
        const signResult = await this.signRequest(isPublicApi, tsISO, method, endpoint, params);
        if (!options.headers) {
            options.headers = {};
        }
        if (!isPublicApi) {
        ...
        return axios(options)
            ...
            if (isRawAPIResponse(response.data) &&
                response.status == 200 &&
     ...
     * Sign request
     */
    async signRequest(isPublicApi, tsISO, method, endpoint, params) {
        const res = {
            requestBody: params,
            method,
            endpoint,
        ...
        const serializedParams = serializeParams(params, method, this.options.strict_param_validation);
        ...
        return {
            ...res,
            sign: await this.signMessage(message, this.apiSecret, 'base64', 'SHA-256'),
        };
    }
    /**
